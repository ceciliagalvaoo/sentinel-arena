...
- A chronological event feed: `SIGNAL → COMMIT (tx) → [waiting] → REVEAL (tx) → ✅/❌`, each row timestamped with both date and time (not just time, a replay or a feed spanning more than one day would otherwise be ambiguous about *which* day an entry happened), with every hash truncated-but-copyable and linked straight to Solscan.

**Comparison chart**, one line per agent, cumulative accuracy over time, on a single shared axis (never a dual-axis chart) with a legend always present, per the project's own charting standard. Each point is plotted at the signal's *detection* time, not its grading time: grading happens in a batch right after a match ends, so keying the X axis off grading time would visually clump an entire match's signals onto nearly the same timestamp regardless of when they actually fired during play, detection time keeps the axis honest to when the agent actually decided. Axis labels and the hover tooltip both show date and time for the same reason as the event feed.
