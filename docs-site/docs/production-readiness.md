...
- **`grades.validation_proof_checked` could get stuck at `false` forever.** The on-chain `validateStatV2` cross-check only ran once, right at the moment `game_finalised` arrived — if TxLINE hadn't yet anchored that day's Merkle root at that exact instant, the check failed and was never retried, permanently blocking the "verified on-chain" badge even though the signal itself had graded correctly (reveal + hash verification are a separate, unaffected step). Found against a real match that had finished only hours earlier in production. Fixed with a periodic sweep (`scheduleValidationRecheckSweep`, every 20 minutes, one owner process — same reasoning as the replay-backfill sweep, since the check result is fixture-wide, not agent-specific) that retries every still-unchecked grade; confirmed against real production data, where it picked up the stuck fixture and flipped it to verified on the very first run after deploy.
